...
{$ELSE}
  Windows, CommCtrl,
{$ENDIF}
  Messages, Classes, Controls, Forms, Graphics, StdCtrls, ExtCtrls,
  Buttons;
...
type
  ...
  private
    ...
    procedure WMLButtonDblClk (var Message: {$IFDEF FPC}TLMLButtonDown{$ELSE}TWMLButtonDown{$ENDIF}); message WM_LBUTTONDBLCLK;
    ...
    procedure CMParentColorChanged (var Message: {$IFDEF FPC}TLMessage{$ELSE}TWMNoParams{$ENDIF}); message CM_PARENTCOLORCHANGED;
    ...
    procedure WMKeyUp (var Message: {$IFDEF FPC}TLMKeyUp{$ELSE}TWMKeyUp{$ENDIF}); message {$IFDEF FPC}LM_KEYUP{$ELSE}WM_KEYUP{$ENDIF};
...
begin
  with Control do
   ...
   and ( Parent.ClientHeight <> 0 ) then
    begin
      if Parent = nil then
        Exit;
      DC := Dest.Handle;
      ...
      GetViewportOrgEx(DC, {$IFDEF COMPILER_2}@{$ENDIF}{$IFDEF FPC}@{$ENDIF}Position);
      ...
      and ( Position.Y <> 0 )then
        Begin
          ...
          Parent.Perform({$IFDEF FPC}LM_PAINT{$ELSE}WM_PAINT{$ENDIF}, DC, 0);
...
procedure TFlatButton.WMLButtonDblClk (var Message: {$IFDEF FPC}TLMLButtonDown{$ELSE}TWMLButtonDown{$ENDIF});
begin
  inherited;
...
procedure TFlatButton.CMParentColorChanged (var Message: {$IFDEF FPC}TLMessage{$ELSE}TWMNoParams{$ENDIF});
begin
  inherited;
  if FUseAdvColors then
  begin
...
procedure TFlatButton.WMKeyDown(var Message: {$IFDEF FPC}TLMKeyDown{$ELSE}TWMKeyDown{$ENDIF});
begin
...
procedure TFlatButton.WMKeyUp(var Message: {$IFDEF FPC}TLMKeyUp{$ELSE}TWMKeyUp{$ENDIF});
begin
  if Message.CharCode = VK_SPACE then
  begin
    if GroupIndex = 0 then
...
procedure TFlatButton.WMMove(var Message: {$IFDEF FPC}TLMMove{$ELSE}TWMMove{$ENDIF});
begin
  inherited;
