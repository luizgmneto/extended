...
Version : 1.0.0.3
Convert XPM to BMP
Colorize method to adjust for toolbutton
...
Known Issues:
-----------------------------------------------------------------------------}
// $Id: JvXPButtons.pas 11400 2007-06-28 21:24:06Z ahuser $
...
uses
  Classes, TypInfo, LCLIntf, lmessages, lresources,
  LCLType, Graphics, Controls, Forms, ActnList, ImgList, Menus,
  JvXPCore, ExtJvXPCoreutils, JvXPCoreUtils;
...
type
  ...
  private
    ...
    FCkGradient,
    FFcGradient: TBitmap;
    FGlyph: {$IFDEF USEJVCL}TJvPicture{$ELSE}TPicture{$ENDIF USEJVCL};
    FHlGradient: TBitmap;
  ...
  protected
    procedure DrawBitmap ( const Image : TPicture  ; const Rect: TRect; const DrawPressed: Boolean); virtual;
    ...
    procedure SetGlyph(Value: {$IFDEF USEJVCL}TJvPicture{$ELSE}TPicture{$ENDIF USEJVCL}); virtual;
    ...
    property Glyph: {$IFDEF USEJVCL}TJvPicture{$ELSE}TPicture{$ENDIF USEJVCL} read FGlyph write SetGlyph;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jvcl.svn.sourceforge.net/svnroot/jvcl/branches/JVCL3_33_PREPARATION/run/JvXPButtons.pas $';
...
//  ControlStyle := ControlStyle - [csDoubleClicks];
  ...
  FGlyph := {$IFDEF USEJVCL}TJvPicture{$ELSE}TPicture{$ENDIF USEJVCL}.Create;
  ...
  begin
    ...
    if GetParentForm(Self) <> nil then
      with GetParentForm(Self) do
        Perform(CM_FOCUSCHANGED, 0, Longint(Pointer(ActiveControl)));
  end;
end;

procedure TJvXPCustomButton.SetGlyph(Value: {$IFDEF USEJVCL}TJvPicture{$ELSE}TPicture{$ENDIF USEJVCL});
begin
  FGlyph.Assign(Value);
  LockedInvalidate;
end;
...
begin
  if Assigned(Action) and (Sender is TCustomImageList) and
    ...
    ((TAction(Action).ImageIndex < (TAction(Action).ActionList.Images.Count)))
   then FImageIndex := TAction(Action).ImageIndex
   else FImageIndex := -1;
  LockedInvalidate;
  Paint;
end;
...
Begin
  ...
  if  Assigned(Image.Graphic)
  and ( Image.Height div 2 < Height )
  and ( Image.Width  div 2 < Width )
  and not Image.Graphic.Empty then
    begin
      if ( Caption <> '' )
       Then
          case FLayout of
            blGlyphLeft:
              Begin
                AX := Rect.Left - (Image.Width + FSpacing);
                AY := (Height - Image.Height) div 2 + Integer(DrawPressed);
              end;
            blGlyphRight:
              Begin
                AX := Rect.Right + FSpacing;
                AY := (Height - Image.Height) div 2 + Integer(DrawPressed);
              end;
            blGlyphTop:
              Begin
                AX := (Width - Image.Width) div 2 + Integer(DrawPressed);
                AY := Rect.Top - (Image.Height + FSpacing);
              end;
            blGlyphBottom:
              Begin
                AX := (Width - Image.Width) div 2 + Integer(DrawPressed);
                AY := Rect.Bottom + FSpacing;
              end;
          end
        else
          Begin
       ...
       with Image do
        Begin
          ...
          Canvas.Draw(AX,AY,Image.Bitmap);
        end;
    end;
...
begin
  ...
  begin
    ...
    if IsSpecialDrawState
    and(   (( dsHighlight in DrawState ) and (FHlGradient.Canvas.Handle<>0))
        or ( not ( dsHighlight in DrawState ) and (FFcGradient.Canvas.Handle<>0)))
     Then
    begin
      try
    ...
    begin
      if (FBgGradient.Canvas.Handle<>0) Then
       Begin
        ...
        if   ( Self.Width - 3 * Offset > 1 + Offset )
        and  ( Self.Height - 3 * Offset > 1 + Offset )
         Then
    ...
    // draw border lines.
    if Enabled
     then Pen.Color := dxColor_Btn_Enb_Border_WXP
     else Pen.Color := dxColor_Btn_Dis_Border_WXP;
    Brush.Style := bsClear;
    ...
    try
      ...
      if (Image.Bitmap.Canvas.Handle <> 0)
        // autogray image (if allowed).
      and FAutoGray and not Enabled then
      ...
      // draw Image - if available.
      if (Image.Bitmap.Canvas.Handle <> 0)
      and Assigned(Image.Graphic)
      and ( Image.Height < Self.Height )
      and ( Image.Width < Self.Width )
      and not Image.Graphic.Empty then
         DrawBitmap(Image,Rect,DrawPressed);
...
//        JvXPFrame3d(Self.Canvas, Rect, clWhite, clBlack, dsClicked in DrawState)
  //    else
    ...
    begin
      LBitmap := TBitmap.Create;
      try
        LBitmap.LoadFromLazarusResource(PChar('JvXPCustomToolButton' + Copy(GetEnumName(TypeInfo(TJvXPToolType),
